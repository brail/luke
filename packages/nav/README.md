...
- **Nothing is imported from `apps/api`.** Configuration arrives through `GetConfigFn`, and the Prisma client is passed in by the caller. `@luke/db` supplies Prisma types; `@luke/core` is declared in the manifest but is not imported by the source today. The direction is enforced by `tools/scripts/check-platform-integrity.ts`, not by convention.
