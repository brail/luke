 ...
 *
 * Logica filtro differenziale:
 * - Se nav_vendors è vuota → full sync (nessuna clausola su Last Date Modified)
 * - Altrimenti → solo record con Last Date Modified > MAX(navLastModified) locale
 *
 * Logica NavSyncFilter:
...
export async function syncVendors(
  ...
  if (lastModified) {
    whereParts.push('[Last Date Modified] > @lastModified');
  }
