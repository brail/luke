...
| `@luke/no-undeclared-workspace-import` | Integrità delle dichiarazioni, non direzione: un riferimento a un package del workspace (nomi letti da `pnpm-workspace.yaml` e dai manifest, `eslint-plugin-luke` incluso) in qualsiasi forma (import, export-from, `import()`, `require()`, `import = require()`, `import('x').T`) è ammesso solo se il `package.json` più vicino dichiara il package in `dependencies` o `devDependencies` (`peerDependencies`/`optionalDependencies` non contano); un package solo in `devDependencies` si usa esclusivamente come tipo, salvo nei file di test. Vieta l'auto-import per nome, i path assoluti e i path relativi che escono dalla directory del package (decisi lessicalmente, senza risoluzione). La direzione ammessa è il controllo P10 di `check-platform-integrity` |
| `@luke/no-restricted-module-references` | `no-restricted-imports` esteso a tutte le forme statiche di riferimento a un modulo (`import()`, template literal senza espressioni, `require()`, `import = require()`, `import('x').T`), sottopath inclusi. Usato per `@luke/core/server` in `apps/web` fuori da `WEB_SERVER_ENTRYPOINT_IMPORTERS` |
