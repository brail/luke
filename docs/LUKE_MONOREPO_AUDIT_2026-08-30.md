...
# Appendix Q — H2 path-aware CI closure: documentation-only pushes (2026-09-06)

Supersedes only the statements in Appendix P that counted the path-aware CI documentation-routing item among the still-open `H2` hygiene work — §P.1's "the `H2` hygiene items introduced alongside `H1` remain open" and "the remaining `H2` items are untouched and still open", and §P.7's "`H2` remains open" — to the extent those statements bundled this one item in. That one item is DONE, recorded below. Every other statement in Appendices A–P stands unchanged, and every finding and evidence record in them remains valid. The rest of the `H2` hygiene batch — `5.1`, `5.3` as narrowed in Appendix M §M.1, `5.4`, `6.3`, and the inert `tools/*` workspace glob — is not closed by this appendix; the whole `H2` batch is not claimed closed, only this one item within it. `P1/P2-08` remains the next architectural item exactly as Appendix O §O.10 and Appendix P record it, and the remaining `H2` hygiene work still operationally precedes it, unchanged in that ordering. `S-01` and the separate documentation workstream are untouched here, as before.

## Q.1 Scope and disposition

- The path-aware CI hygiene item — routing a documentation-only push on `develop-2.2` away from `ci.yml`'s full `checks`/`browser`/`integration`/`migrations` pipeline and into a dedicated drift check in a new `docs.yml` — is DONE.
- It is a CI/control-plane optimization: which GitHub Actions workflow observes a given push, and what that workflow runs. It changes no application behavior, is not a numbered Cycle in the `§A.4` architectural sequence, and supersedes nothing beyond the narrow correction above.
- No runtime code changed. No package manifest version moved. No dependency was added, removed, or bumped. No Prisma schema, migration, or database object changed. No Docker image, `entrypoint.sh`, or deployment/Portainer contract changed.
- The remaining `H2` hygiene items — `5.1`, `5.3` as narrowed, `5.4`, `6.3`, and the inert `tools/*` workspace glob — are not touched by this closure and remain open.
- `S-01`, the separate documentation workstream, and `P1/P2-08` are unrelated to this item and remain exactly as recorded before it.

## Q.2 Measured premise

The investigation behind this item measured 63 push intervals over the preceding 30 days on `develop-2.2`, of which 25 — approximately 40% — changed only paths inside what became the documentation ownership surface. Those 25 intervals accounted for approximately 2 h 41 min of cumulative CI wall time and approximately 4.9 runner-hours across the jobs `ci.yml` runs on push. That 40% share is inflated by the audit cadence itself — this repeated append-only-audit workflow produces a disproportionate share of documentation-only pushes relative to steady-state development — and was expected to settle to approximately half that share once the cadence normalizes. GitHub-hosted runner minutes on this repository cost $0 in this measurement window; the benefit is feedback latency and unnecessary execution avoided, not monetary savings.

**Withdrawn premise.** An earlier reading of the same window cited failed run `33915244785` as evidence that documentation-only pushes had previously served as accidental, useful reruns of a flaky pipeline. That reading does not survive a check of the run's own interval: the push behind `33915244785` was not documentation-only — its interval spans 261 changed files, including `.github/workflows/*`, `.github/actions/setup-workspace`, `.semgrep/rules/*`, and Prisma schema/migration files. There is therefore no evidence, from this investigation, that documentation-only pushes ever served as useful accidental reruns; that claim is withdrawn and plays no part in the reasons for this item.

## Q.3 Final architecture

`ci.yml`'s `push` trigger carries an exact, 15-pattern `push.paths-ignore` documentation-ownership allowlist (`CI_DOCUMENTATION_PATHS_IGNORE` in `tools/scripts/check-workflow-paths.ts`): `docs/**/*.md`, `docs/**/*.pdf`, `README.md`, `CLAUDE.md`, `lessons.md`, `lessons-archive.md`, `API_SETUP.md`, `APP_CONFIG.md`, `OPERATIONS.md`, `SETUP_STATUS.md`, `INTEGRATIONS_ROADMAP.md`, `apps/*/README.md`, `packages/*/README.md`, `tools/README.md`, and `.claude/skills/**/*.md`. It applies to `push` only; `pull_request` stays unfiltered so the required checks on `main` always report; `workflow_call` is retained so `release.yml` can keep reusing this workflow as its verify gate. `security.yml` and `release.yml` carry no path filter of any kind and remain path-blind, exactly as before this item.

`docs.yml` is push-only, on `main`, `develop-*`, and `release/*`, observing `docs/**` and `**.md` — deliberately broader than `ci.yml`'s ignore list, so Markdown outside the documentation-ownership surface still gets observed by something. It declares exactly one job, `Documentation drift`, which is not a required check on `main`; the job checks out the repository (`actions/checkout`), runs `./.github/actions/setup-workspace`, and executes the exact inline step `run: pnpm check:drift` — no block-scalar form, no `if:`, no `continue-on-error:` at job or step level. No third-party GitHub Action, external classifier service, new dependency, or GitHub-side branch-protection ruleset change was introduced: the whole mechanism is two workflow files plus one checker script.

Fail-closed behaviour: a push skips `ci.yml`'s full pipeline only when every one of its changed paths matches an entry in the 15-pattern allowlist; a single changed path outside it — a new Markdown location, `CHANGELOG.md`, anything under `.github/**`, a non-Markdown file under `docs/`, a test fixture — runs the full pipeline. `CHANGELOG.md` is deliberately excluded because it is release control, not documentation ownership; workflow files, Markdown living inside a source tree (e.g. `apps/web/src/content/help.md`), and test fixtures are likewise excluded and continue to trigger full CI whether or not they happen to end in `.md` — a test fixture belongs under its own test tree, never under `docs/`. Every path the CI allowlist ignores must also be observed by `docs.yml`'s broader `paths`, and every allowlist entry must match at least one currently tracked file — both machine-checked (§Q.5). Because the allowlist decides when CI does not run, the checker treats the two workflow files and its own pinned constant as one unit to change together, never separately.

## Q.4 Rejected designs and simplification

Rejected during design: a single global Markdown exclusion (`**.md` in `ci.yml`'s ignore list) — refused because it would silently fail open the instant any future runtime- or test-adjacent Markdown file is added anywhere in the tree; filtering `pull_request` the same way — refused because a path-filtered required check reports `Pending`, not `Passed`, on a documentation PR, blocking merge rather than skipping harmlessly; filtering `security.yml` or `release.yml` — refused because scanning and the release gate must never depend on which paths changed; a bespoke path-classifier job, a third-party path-filter GitHub Action, path-aware local pre-push hooks, and adopting a remote build cache — all considered and rejected as disproportionate to a problem two workflow files and one pinned allowlist solve directly.

The implementation grew, before Push A, to 567 lines in the checker and 577 in its test file. A `/simplify` pass reduced that to 404 and 417 lines while preserving the same behavioral contract. An independent Opus `/code-review` pass against that simplified state then found real defects, not style complaints: the Docs job contract did not inspect or reject `if:`/`continue-on-error:` controls at all, allowing a sole-observer job to be skipped or to mask failure, and the Docs trigger contract did not enforce the complete approved trigger set. Final hardening against those findings brought the checker to its current, verified 460 lines and its test file to 461 lines, with 24 focused tests (`node --test`, via `pnpm test:tools`) — sizes reviewed as proportionate to the number of independent fail-closed invariants pinned, not accepted merely because the suite was green.

## Q.5 Checker evidence

The 24-test suite in `tools/scripts/check-workflow-paths.test.ts` collectively provides red mutations for every enforced invariant and green controls for the accepted forms and path semantics; it covers: a missing, widened, or duplicated `ci.yml` allowlist entry; an additional `push.paths` or any `pull_request` path filter on `ci.yml`; a removed `workflow_call`; any path filter at all on `security.yml` or `release.yml`; a `docs.yml` trigger path set narrower or wider than the pinned pair; any Docs trigger beyond `push`, and the loss of `push` itself; a missing, extra, or renamed `Documentation drift` job, including a rename colliding with a `ci.yml` job name; a missing checkout step, a missing `setup-workspace` step, or a `run:` not exactly the inline `pnpm check:drift` (block-scalar form included); an `if:` or `continue-on-error:` at job or step level, in every position and spelling recognized as live risk; a CI-ignored tracked file `docs.yml` does not observe; a dead allowlist entry matching no tracked file; and malformed or unreadable workflow shapes, which fail as an error rather than a silent pass. A dedicated test asserts GitHub's own path-filter semantics against a concrete path table (documentation-owned and CI-skipped; Markdown outside the surface, CI-run but Docs-observed; neither), including the documented `**/` zero-directory-match case.

Final local evidence before Push A: `pnpm lint:tools` clean; `pnpm typecheck:tools` clean; `pnpm test:tools` at 246/246; `pnpm check:drift` green across all five checkers; `git diff --check` clean.

## Q.6 Push A evidence

Commit `d27f7ea646790fd9dc88dd4d3b1aa20fe7c7ad2f`, `ci: route documentation-only pushes to drift checks`, parent `5d90ba8aeaa3a0aa878843afb85d8e87243b0de0`. Scope: `.github/workflows/ci.yml`, `.github/workflows/docs.yml`, `package.json`, `tools/scripts/check-workflow-paths.ts`, `tools/scripts/check-workflow-paths.test.ts` — 979 insertions, 1 deletion (`+22/-0`, `+35/-0`, `+1/-1`, `+460/-0`, `+461/-0` respectively).

Remote CI, run `34023214459`: **success** — all four jobs green (`Lint, TypeCheck & Unit Tests`, `Migrations`, `Integration Tests`, `Browser Component Tests`). Remote Security, run `34023214433`: **success** — `gitleaks`, `semgrep`, and `osv-push` succeeded as real jobs; `osv-weekly`, `osv-weekly-release-train`, and `notify-on-failure` show `skipped`, correctly, being schedule-only or failure-only triggers inert on a push event. Push A triggered CI in full because its changed paths — two workflow files, the checker, its test file, `package.json` — fall entirely outside the documentation allowlist it was introducing; the mechanism gated its own first push. `docs.yml` did not run: no changed path in this commit matches its `docs/**`/`**.md` trigger. `Release` did not run: no tag was pushed. No rerun, dispatch, bypass, or compensating action occurred against either run.

This pushed commit's message carries a trailer, `Co-Authored-By: Claude Opus 5 (1M context) <noreply@anthropic.com>`, added despite the no-`Co-Authored-By` convention already standing in LUKE before this commit. It was noticed only after the commit had been pushed and remotely verified. Rewriting already-pushed history for commit metadata alone was rejected; the trailer is recorded here only for historical accuracy, has no runtime or audit-evidence effect, and this appendix does not propose amending or rewriting Push A.

## Q.7 Push B evidence

Commit `bcc794267b39d7545672d3b6dad330ca0e8bdc99`, `docs: document the documentation-only CI path`, parent `d27f7ea646790fd9dc88dd4d3b1aa20fe7c7ad2f`. Scope: `CLAUDE.md` only, one append hunk, 20 insertions, 0 deletions. This commit's message carries no `Co-Authored-By` trailer, conforming to LUKE's standing no-trailer convention.

Remote Docs, run `34023760080`: **success** — the job's three load-bearing steps (checkout, `setup-workspace`, `Docs & skills drift`) all succeeded, and its log shows all six current drift-checker outputs in sequence: `[skill-integrity] ok`, `[docs-integrity] ok`, `[platform-integrity] ok`, `[tsconfig-integrity] ok`, `[workflow-branches] ok`, `[workflow-paths] ok`. Remote Security, run `34023759966`: **success** — `gitleaks`, `semgrep`, and `osv-push` succeeded as real jobs; the same three schedule/failure-only jobs skipped intentionally. `CI` is absent from the run list for this exact SHA; `Release` is absent.

This is the acceptance proof the item exists for: a push whose only changed path is `CLAUDE.md` — inside the allowlist — ran the replacement drift lane in full and did not run the full CI pipeline, while Security still ran unconditionally.

## Q.8 Honest boundaries

- The documentation-ownership allowlist is a maintained architectural classification, not a derived one: if a currently allowed path later becomes a genuine runtime, build, or test input, the allowlist and the checker's pinned constant must be updated together, by hand — nothing detects that shift on its own.
- The checker reads the current tracked tree and the two workflows' pinned line-oriented shapes; it is not a general YAML parser or a GitHub Actions semantics engine.
- The exact inline `run: pnpm check:drift` form is a deliberate part of the contract; an equivalent `run: |` block-scalar form is rejected by design, so a future reformat of the step is caught rather than silently accepted.
- Step ordering within the Docs job is not machine-enforced: an out-of-order step fails visibly on the runner, a different failure mode than the silent-success class this checker exists to close, and was judged not to need its own invariant.
- GitHub's documented `**` path-filter semantics are exercised by the checker's own test table for `.claude/skills/**/*.md`, but no real push in this closure exercised a skill-Markdown-only change — Push B's only changed path was root `CLAUDE.md`. No push was manufactured to close that gap; the next naturally occurring skill-Markdown-only push can supply that operational observation.
- GitHub Actions' own changed-file diff has an upstream limit above roughly 3,000 files, beyond which its path-filter evaluation is documented to behave differently; that ceiling is a residual of the platform, not something this checker or these workflows compensate for.
- The local `.husky/pre-push` hook remains path-blind: it runs its full gate chain on every push regardless of which paths changed, including the documentation-only Push B (§Q.7). This item changes only what GitHub's own Actions runs, not local hook behavior.
- `docs.yml`'s drift check does not replace `security.yml`: a documentation-only push still gets Semgrep, gitleaks, and OSV scanning in full, exactly as before.
- This appendix cannot cite Push C's own future Docs/Security run IDs — Push C is this audit commit itself, not yet pushed at the time of writing. Any such run IDs, if useful, belong in the next natural appendix, not here.
