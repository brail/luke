...
describe('the requirements shown come from the policy', () => {
  it('with everything on it lists them all', () => {
    expect(keys(FALLBACK_PASSWORD_POLICY)).toEqual(['length', 'uppercase', 'lowercase', 'digit', 'special']);
  });

  it('a requirement switched off disappears from the list instead of staying unmet', () => {
  ...
  it('the length shown is the configured one, not a constant', () => {
  ...
  it('the symbols requirement names the allowed characters', () => {
  ...
  it('the fallback uses the server class, not a copy', () => {
...
describe('the verdict follows the policy', () => {
  it('rejects below the configured length and accepts above', () => {
  ...
  it('switching a requirement off makes the same password valid', () => {
...
describe('quali caratteri contano come speciali', () => {
  ...
  for (const ch of ['~', '`', '€', ' ']) {
    it(`does not accept ${JSON.stringify(ch)}, like the server`, () => expect(withChar(ch)).toBe(false));
  }
});

describe('empty password and confirmation', () => {
  it('an empty password is not valid and says what is missing', () => {
  ...
  it('when it is not valid there is always something to show', () => {
  ...
  it('without a confirmation field it does not add the match check', () => {
