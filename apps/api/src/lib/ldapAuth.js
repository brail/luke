...
import { getLdapConfig } from './configManager';
 ...
 */
export async function authenticateViaLdap(prisma, username, password) {
    let client = null;
    try {
        ...
        const isValidCredentials = await verifyUserCredentials(client, userDN, password);
        ...
        const user = await createOrUpdateUser(prisma, username, userDN, role, userAttributes);
        console.log(`LDAP authentication successful for user: ${username}, role: ${role}`);
        return user;
    }
    catch (error) {
 ...
 */
async function searchUser(client, config, username) {
    return new Promise((resolve, reject) => {
        ...
        client.search(config.searchBase, {
            ...
            res.on('end', () => {
                console.log(`LDAP search completed. Found user: ${foundUser ? foundUser.dn : 'none'}`);
                resolve(foundUser);
            });
        });
    });
 ...
 * Verifica le credenziali dell'utente
 */
async function verifyUserCredentials(client, userDN, password) {
 ...
 * Cerca i gruppi dell'utente
 */
async function searchUserGroups(client, config, userDN) {
 ...
 */
function determineUserRole(userGroups, roleMapping) {
 ...
 * Crea o aggiorna l'utente nel database
 */
async function createOrUpdateUser(prisma, username, userDN, role, userAttributes) {
    ...
    if (user) {
        ...
        console.log(`User ${username} already exists, syncing firstName/lastName from LDAP`);
        ...
        if (user.firstName !== firstName || user.lastName !== lastName) {
            ...
            console.log(`Updated firstName/lastName for user ${username}: ${firstName} ${lastName}`);
        }
        ...
        user = await prisma.$transaction(async (tx) => {
            ...
            console.log(`Created new LDAP user: ${username} with role ${role}, firstName: ${firstName}, lastName: ${lastName}`);
            return newUser;
        });
    }
    return user;
