...
| `@trpc/server`, `@trpc/client`, `@trpc/react-query`    | wire format and router typing across `apps/api`, `apps/web`, root                                                | bump the family together, then `pnpm --filter @luke/api build` before typechecking `apps/web` — otherwise web still resolves the previous `dist` contract                                                                                             |
