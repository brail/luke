...
test('an ADR missing from the index fails', () => {
  ...
  assert.ok(
    problems.some(p => /002-second\.md` does not appear in the index/.test(p.message)),
...
test('a tracked ADR deleted from the working tree makes its index entry dangle', () => {
  ...
  assert.ok(
    problems.some(p => /index entry `002`.*not an ADR in the corpus/s.test(p.message)),
...
test('an ADR index deleted from the working tree is reported, not skipped', () => {
  ...
  assert.ok(
    problems.some(p => /ADRs in the working tree but the index is missing/.test(p.message)),
